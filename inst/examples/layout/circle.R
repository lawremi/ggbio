...
rearr <- read.xlsx("/home/tengfei/Downloads/ng.936-S3.xlsx", sheetIndex = 1, rowIndex = 20:702)
rearr.bk <- rearr
rearr <- rearr.bk
rearr <- rearr[!rearr$chr1 %in% c(23, 24) & !rearr$chr2 %in% c(23, 24), ]
## rearr$chr1 <- paste("chr", rearr$chr1, sep = "")
## rearr$chr2 <- paste("chr", rearr$chr2, sep = "")
nms <- colnames(rearr)
...
gr.cur <- gr.crc4 <- gr[values(gr)$individual == "CRC-4"]
