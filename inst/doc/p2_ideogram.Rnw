% \VignetteIndexEntry{Part 2: How to make ideogram plot}
% \VignetteDepends{} 
...
\sfbox{ NOTE: ideogram introduced in this vignette, aim to be embedded in
  \textbf{tracks}, it is a very special class called 'ideogram', if you want to
  plot one or more chromosomes that you can add flexible data(layer by layer) on
  it, you have to check karyogram overview which is included in vignette
  'overview'. }
