...
setMethod("geom_alignment", "data.frame", function(data,
                                                   stat = c("stepping", "identity"),
                                                   args,rect.height = 0.4){

  data.ori <- transformDfToGr(data)
  ...
  if(stat == "identity"){
    args.aes <- args.aes[!(names(args.aes) %in% c("data"))]
...
setMethod("geom_alignment", "GRanges", function(data, ..., stat = c("idenity", "stepping")){
  if(stat == "stepping")
    p <- stat_stepping(data, ..., geom = "alignment")
  if(stat == "identity")
    p <- stat_identity(data, ..., geom = "alignment")
  p
})
