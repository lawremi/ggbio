...
## Since the default is geom rectangle, even though it's looks like segment
...
```{r}
## since default is geom rectangle, even though it looks like a segment
## we still use both fill/color to map colors
autoplot(dn, layout = "karyogram", aes(color = exReg, fill = exReg), alpha  = 0.5) +
  scale_color_discrete(na.value = "brown")
```

Well, sometimes we have too many values, we want to separate them by groups and show them at different height, below is a hack for that purpose and in the next section, we will introduce a more flexible and general way to add data layer by layer.
