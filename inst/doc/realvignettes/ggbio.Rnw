...
\section{Quick start}
This chapter will give your a general overview about \ggbio{} and will guide you
to approperiate chapter as possible as it could be.


\autoplot{} is the generic function which support most core \Bioc{} objects, try
to make different types of graphics for specific object. Please check
Chapter\ref{chapter:autoplot} for \autoplot{}.
...
## equal to 
autoplot(gr, stat = "coverage", geom = "polygon")
...
## fix me
## autoplot(gr, aes(y = value)) + stat_aggregate()
## autoplot(gr, aes(y = value), stat = "aggregate")
...
\chapter{Visualize single chromosome}
\section{Introduction}
...
@ %def 
\chapter{Visualize gtf/gff files}
\chapter{Visualize bam files}
\chapter{Visualize matrix-related objects}
\chapter{Visualize VCF files}
\chapter{Visualize splicing events}
