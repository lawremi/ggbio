...
when they try to show isoforms or gene model. More advanced useage is
...
\begin{figure}[h!t!p!b]
  ...
  \caption{geom chevron. how to use parameters to contrl it.}
  \label{fig:chevron3}
\end{figure}
...
No facet support, just simply viewing the intervals and coverage.
...
summarization methods for viewing these types of data. We support
three geoms \emph{point, line, segment} and five types of smoothing or
summarization methods,\emph{raw, viewMaxs, viewMins, viewSums,
...
use the following four method, you could pass extra auguments, such as
\Rfunarg{lower} to control how to smooth the data.

For \Robject{RleList}, it automaticaly facet them by list, controled
...
@ 
<<Rlelist-qplot, eval = FALSE, fig = FALSE>>=
set.seed(1)
...
qplot(xRleList, geom = "line", facetByRow = TRUE)
...
Figure \ref{fig:rletlist} shows multiple sample comparison.
\begin{figure}[h!t!p!b]
  \centering
...
\begin{figure}[h!t!p!b]
  \centering
  \includegraphics[width = 0.7\textwidth]{bamplot.png}
  \caption{Tracks of different geom of bam file, in the order of gapped.pair, gapped.pair with junction, full, coverage.line, coverage.polyhon, mismatch.summary.}
  \label{fig:bam-plot}
\end{figure}
...
\item[tx] Transcirpts isoforms coposed of exons.
\end{description}
...
@ 
 ...
 \begin{figure}[h!t!b!p]
   \centering
   \includegraphics[width = 0.8\textwidth]{txdb.png}
...
In package \ggbio{}, We also make individual graphic for special case instead of 
using a generic function \qplot{}, when it meets following conditions:
...
\item This function doesn't return a ggplot object. Usually
  ...
  \item The object is not qutie clear, or we don't want it to dispatch
...
set of utilities to generate those graphcis.
...
RNA-seq bam file directly and summerize the junction read, then plot
the result directly as graphic.

Internally we use biovizBase:::spliceSummary for simple counting by
overlaped junction reads which cover consecutive two exons of each
model in weigthed or unweighted way, but we encourage users to use
...
p <- plotSpliceSum(bamfile, exons.rbm17)
p <- plotSpliceSum(bamfile, exons.rbm17, weighted = FALSE, offset = 0.01)
p <- plotSpliceSum(bamfile, txdb, which = genesymbol["RBM17"])
p <- plotSpliceSum(bamfile, txdb, which = genesymbol["RBM17"], offset = 0.01)
@ %def 
% need a grphic here
\begin{figure}[h!t!p!b]
  \centering
\includegraphics[width = 0.7\textwidth]{plotSpliceSum.png}  
...
exons <- exons(txdb)
exons.rbm17 <- subsetByOverlaps(exons, genesymbol["RBM17"])
plotFragLength(bamfile, exons.rbm17[-13])
gr <- getFragLength(bamfile, exons.rbm17)
plot(res$pos,res$isize)
lst <- gr
...
compact. Simply view the statistics across the geonme and align
...
But we suport both \emph{point} and \emph{line} geom.
...
qplot(gr.snp, geom = "point", y = pvalue, scales = "free", space = "free", 
      facet = group ~ seqnames)
## plottting
...
data, and show them as \emph{Paralell Coordinates Plots}. This is one
special layout which represent the data in a nice manner and also keep
the genomic structure information. With abliity of \Rfunction{tracks},
...
@ 
<<plotRangesLinkedToData, eval = FALSE>>=
