...
\section{Data Sets}
Most data sets used in this packages came from package \biovizBase{},
\Robject{genesymbol} is a \Robject{GRanges} object which contains all
the genes position in hg19, its symbol name, and emsembl name. BamFile
used in these examples is from SRA(Short Read Archive), Accession:
SRR027894 and subset the gene at chr10:6118023-6137427, which within
gene RBM17. this sample contains junction reads but not much
paired-end read, so for fragment length estimation section, please use
some data that contains paired-end reads, otherwise the function will
give you some error message. We will come up with a new data set
later.
