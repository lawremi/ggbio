...
setMethod("qplot", signature(data = "GRanges"), function(data, x, y,...,
  ...
  p <- switch(geom,
              line = {
                if(!missing(facet.gr)){
                  lst.rc <- rangesCentric(data, facet.gr)
                  data <- lst.rc$gr
                  facet.gr.r <- lst.rc$which
                }
...
setMethod("qplot", "BamFile", function(data, ..., which,
  ...
  if(geom == "fragment.length"){
    ...
    ## ga.r <- as(ga, "GRanges")
    ga.r <- yield(data, raw, which = which)
...
setMethod("qplot", "character", function(data, x, y, ..., ref, which,
                                         ...
                                         geom = c("gapped.pair",
                                           ...
                                           "coverage.line",
                                           "coverage.polygon")){
  if(substr(data, start = nchar(data)-3, stop = nchar(data)) == ".bam")
    bf <- BamFile(data)
  else
...
setMethod("qplot", c("BSgenome"), function(data,  name, ...,
  ...
  geom <- match.arg(geom)
  p <- ggplot(df, ...)
  if(missing(baseColor)){
    baseColor <- getOption("visnabBase")$baseColor
...
## ======================================================================
##        For "Ideogram" a special case for GRanges??
## ======================================================================
## ok, let it be a sepecial case in GRanges
## should be able to show zoomed region
plotIdeogram <- function(obj, zoom, hotRegion){
  if(!isIdeogram(obj))
    stop("Please including provide a GRanges object
          including 'name' and 'gieStain' as elementMetadata")
