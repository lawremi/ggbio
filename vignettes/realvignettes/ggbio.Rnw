...
We provides a simple navigatino for your graphics
\begin{itemize}
\item zoom: put a factor inside and you can zoom in or zoom out
\item nextView: switch to next view
\item prevView: switch to previous view
\end{itemize}
try following code to move and zoom on your view
<<eval = FALSE>>=
p.txdb + zoom()
p.txdb + zoom(1/8)
p2 <- p.txdb + nextView()
...
<<>>=
gr17 <- GRanges("chr17", IRanges(41234370, 41234570))

library(BSgenome.Hsapiens.UCSC.hg19)
bg <- BSgenome.Hsapiens.UCSC.hg19
p.bs <- autoplot(bg, which = gr17) 
p.bs + zoom(1/6)
...
<<>>=
fl <- system.file("extdata", "wg-brca1.sorted.bam", package  = "biovizBase")
library(Rsamtools)
p.mis <- autoplot(fl, bsgenome = bg, which = gr17, stat = "mismatch")
p.mis
@
...
fl <- system.file("extdata", "17-1409-CEU-brca1.vcf.bgz", package="biovizBase")
vcf <- readVcf(fl, "hg19")
## show sample 1
autoplot(vcf)
vr <- as(vcf[, 1:2], "VRanges")
vr <- renameSeqlevels(vr, value = c("17" = "chr17"))
autoplot(vr, which = gr17, geom = "rect", arrow = FALSE)
p.vr <- autoplot(vr, which = gr17, geom = "text") 
p.vr

@
\section{Building your tracks}
<<fig.width = 7, fig.height = 5.5>>=
tks <- tracks(mismatch = p.mis, dbSNP = p.vr, reference = p.bs, gene = p.txdb, 
              heights = c(3, 3, 1, 3)) + xlim(gr17) 
tks + zoom(1/6) + theme_tracks_sunset()
@
