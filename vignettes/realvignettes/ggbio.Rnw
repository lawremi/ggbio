...
In the example below we plot the gene model of the gene PHKG2.
...
Alternatively, we could specify a genomic region and fetch all transcripts
overlapping that region (also partially, i.e. with a part of an intron or an
exon).
