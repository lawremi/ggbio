...
system is still reproducible, the html is produced from /inst/md_example file, a
...
Extensive details about \ggplot{} are available in the on-line documentation, or
the web site \url{http://had.co.nz/ggplot2/geom_point.html}. The key pieces of
...
In the spirit of being general and object-oriented, following the syntax of
\ggplot{}, we redefine the \emph{quick plot} function \qplot{} in \ggplot{}
package to a new generic function \autoplot{}, as a \textbf{S4} generic
function. Then the \autoplot{} function could automatically and appropriately
plot different R objects. For each object type a new \textbf{geom} is defined.

In the following section, we will introduce how to use \autoplot{} to plot
different types of data to generate different displays.

\textbf{More updated sections comming}

\section{Update since bioc 2.10}
\begin{itemize}
  \item qplot -> autoplot
  \item more stable tracks()
  \item facet\_gr argument in autoplot, removed, all changed to facets
    \item ggplot2 version updated to 0.9
\end{itemize}
\section{Autoplot Generic Function}

Begin with new \ggplot{} 0.9, there is a generic function defined inside used
for extending ggplot2 and creating particular graphics for different class. so
we drop the generic \qplot{} function in previous version of \ggbio{}. So please
notice:

\begin{itemize}
\item \qplot{} is not supported anymore in \ggbio{}.
\item \autoplot{} is new generic in \ggbio{}. 
\item Both \qplot{} or \autoplot{} will return a \Rclass{ggplot} object. So all
  add-on featurs exists in \ggplot{} should work in \ggbio{} too.
\end{itemize}

So users could easily tell the differnce between two API.

\subsection{Autoplot for \Rclass{GenomicRanges} object}
...
@ %def 
% need a table
\subsection{Autoplot for \Rclass{TranscriptDb} object}
\subsection{Autoplot for \Rclass{ExpressionSet} object}
\section{Tracks}
\section{Layout}
