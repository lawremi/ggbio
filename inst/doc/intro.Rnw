...
\begin{figure}[h!p!t]
  \centering
@ 
<<gapped-plot, fig = TRUE>>=
library(Rsamtools)
bamfile <- system.file("extdata", "ex1.bam", package="Rsamtools")
galn1 <- readBamGappedAlignments(bamfile, use.names = TRUE)
galn1.seq1 <- galn1[seqnames(galn1) == "seq1"]
p1 <- qplot(galn1.seq1)
p2 <- qplot(galn1.seq1, geom = "full")
...
\begin{figure}[h!p!t]
  \centering
@ 
<<gapped-plot-zoom, fig = TRUE>>=
p1 <- p1 + scale_x_continuous(limits = c(500, 800))
p2 <- p2 + scale_x_continuous(limits = c(500, 800))
grid.arrange(p1, p2, ncol = 1)
@ %def 
...
\section{Future Plan}
\begin{itemize}
\item Manhattan Plots for SNP data(can be achieved by facet\_gr for now, but
