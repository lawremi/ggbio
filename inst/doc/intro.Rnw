...
p3 <- plotGrandLinear(gr.snp, y = pvalue, geom = "point") +
...
p4 <- plotGrandLinear(gr.snp, y = pvalue, geom = "point") +
...
@ 
<<overview-facet, eval = FALSE, figure = FALSE>>=
...
@ 
<<frag-length, fig = FALSE, eval = FALSE, fig = FALSE>>=
...
@ 
<<plotRangesLinkedToData, eval = FALSE, fig = FALSE>>=
exons <- exons(txdb)
...
\begin{figure}[h!t!p!b]
  \centering
  \includegraphics[width = 0.6\textwidth]{linked.png}  
...
\begin{figure}[h!t!p!b]
  \centering
  \includegraphics[width = 0.6\textwidth]{linked-anno.png}  
